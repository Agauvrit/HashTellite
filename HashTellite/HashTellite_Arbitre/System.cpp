...
*/
bool System::locationViewable(Satellite* s, Coordinates* c, Location* l) {

	// Calcul des bornes de vision du satellite
	long minAbs = c->getLatitude() - s->getRadius();
	long maxAbs = c->getLatitude() + s->getRadius();
	long minOrd = c->getLongitude() - s->getRadius();
	long maxOrd = c->getLongitude() + s->getRadius();
...
/**
*	Calcul du nombre de points grace au fichier de sortie
*	@param s
*			Chemin du fichier de sortie
*	@return Le score associé
*/
int System::comptePoint(std::string s) {

	int nbPhotos, idsat, tour, score = 0, nbPicTak = 0;
	...
	// POur chaque photos prise
	for (int i = 0; i < nbPhotos; i++) {

		// Récupération des informations propres à la photo
		std::getline(file, ligne);
		stringstream ss2(ligne);
		ss2 >> latitude >> longitude >> tour >> idsat;
		
		// Recherche de la location visé
		std::map<long, std::map<long, Location*>>::iterator it1 = mapLocs.lower_bound(latitude);
		if (it1 != mapLocs.end()) {
			std::map<long, Location*>::iterator it = it1->second.lower_bound(longitude);
			// Si la location existe
			if (it != it1->second.end()) {
				// Vérification que l'image a été prise dans un interval de temps légal
				if (it != it1->second.end()) {
					// Vérification que l'image a été prise dans un interval de temps légal
					if (iscollectionTakeable(this->liste_collections.at(it->second->getIdCol()), tour)) {
						it->second->setPhotoTaken(true);
					}
					else {
						problem = true;
					}
	...
	// Si la location visé n'est pas légal
	if (problem) {
		return 0;
	}
	else {
		// Pour chaque collection
		for (size_t j = 0; j < this->liste_collections.size(); j++) {
			// Vérification du nombre de photos prise
			for (auto l : this->liste_collections.at(j)->getListePictures()) {
				if (l->getPhotoTaken())
					nbPicTak++;
			}
			// Si toutes les locations d'une collection on été prise en photos, on ajoute le score
			if (nbPicTak == this->liste_collections.at(j)->getListePictures().size()) {
				score += this->liste_collections.at(j)->getNbPoints();
			}
			nbPicTak = 0;
		}
	}
